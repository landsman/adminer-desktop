...
# point so its version pins stay in one place. Toolchain first, then `fetch` for the
# downloads (adminer + designs), so one command sets a fresh checkout up completely.
install:  ## Install everything: dev toolchain (node + composer deps, e2e browser) + downloads
	mise run install
	$(MAKE) fetch
